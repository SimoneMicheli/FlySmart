			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if(Integer.parseInt(view.labelNumeroPasseggero.getText())<view.voloSelezionatoPasseggeri.getPostiDisponibili()){
					if(Integer.parseInt(view.labelNumeroPasseggero.getText())<8){
						...
						JOptionPane.showMessageDialog(null,"Non Ë possibile effettuare prenotazioni per pi˘ di 8 persone","Errore", 0); 
					}
				}else{
		...
		view.buttonPasseggeriConfermaPrenotazione.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent arg0) {
				if(view.controllaCampi() || (view.campiVuoti() && view.listaPasseggeri.size()!=0)){ //se sono pieni oppure vuoti
					view.passeggeroSuccessivo(); 
					if (JOptionPane.showConfirmDialog(null,"Vuoi confermare la spesa di "+view.prezzoTotaleVolo+" ‚Ç¨?","Conferma prenotazione passeggeri",JOptionPane.YES_NO_OPTION,JOptionPane.NO_OPTION) == JOptionPane.OK_OPTION) {
						try {
...
<<<<<<< HEAD
							JOptionPane.showMessageDialog(null,"I posti non sono pi√π disponibili","Errore", 0);
=======
							JOptionPane.showMessageDialog(null,"Attenzione: i posti non sono pi˘ disponibili","Errore", 0);
>>>>>>> refs/heads/gui
						}
					}else{
